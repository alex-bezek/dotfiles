...
install_macos_tools() {
  ...
  # Use Brewfile if available (covers formulae, casks, fonts, VS Code extensions)
  if [[ -f "$DOTFILES_DIR/Brewfile" ]]; then
    echo "📦 Installing from Brewfile (this may take a while)..."
    brew bundle --file="$DOTFILES_DIR/Brewfile" --no-lock || echo "⚠️  Some Brewfile items failed"
  else
...
setup_git_platform_config() {
  echo "🔧 Setting platform-specific git config..."
  # Platform-specific git settings go in ~/.config/git/config (included by
  # the main git/config via [include]). This avoids dirtying the repo.
  local platform_config="$HOME/.config/git/config"
  mkdir -p "$(dirname "$platform_config")"

  cat > "$platform_config" <<'GITCFG'
# Auto-generated by dotfiles install.sh — platform-specific git settings
# Do not edit manually; re-run install.sh to regenerate.
GITCFG

  # Hooks path (portable — uses $HOME)
  git config -f "$platform_config" core.hooksPath "$HOME/.config/git/hooks"

  if [[ "$OS" == "macos" ]]; then
    git config -f "$platform_config" credential.helper osxkeychain
    git config -f "$platform_config" core.editor "code --wait"
  else
    # Linux: use built-in credential cache (15 min timeout)
    git config -f "$platform_config" credential.helper "cache --timeout=900"
    git config -f "$platform_config" core.editor "nvim"
  fi

  # git-lfs: only enable if installed (required=true breaks git when lfs is missing)
  if command -v git-lfs &> /dev/null; then
    git config -f "$platform_config" filter.lfs.required true
    git config -f "$platform_config" filter.lfs.clean "git-lfs clean -- %f"
    git config -f "$platform_config" filter.lfs.smudge "git-lfs smudge -- %f"
    git config -f "$platform_config" filter.lfs.process "git-lfs filter-process"
  fi
...
setup_git_ignore() {
  ...
  ln -sf "$DOTFILES_DIR/git/ignore" "$dest"
  echo "  global gitignore → $dest"
}
...
setup_symlinks() {
  ...
  # git-lfs: configured in setup_git_platform_config to avoid dirtying repo
  ...
  # VS Code settings (macOS path; Linux/Codespaces use different paths)
  if [[ "$OS" == "macos" ]]; then
    local vscode_dir="$HOME/Library/Application Support/Code/User"
  else
    local vscode_dir="$HOME/.config/Code/User"
  fi
  if [[ -d "$(dirname "$vscode_dir")" ]] || command -v code &> /dev/null; then
    mkdir -p "$vscode_dir"
    ...
    echo "  VS Code settings → $vscode_dir"
  fi
  ...
  if [[ -f "$DOTFILES_DIR/ssh/config" ]]; then
    mkdir -p "$HOME/.ssh"
    chmod 700 "$HOME/.ssh"
    # Don't clobber — use symlink so updates propagate
    ln -sf "$DOTFILES_DIR/ssh/config" "$HOME/.ssh/config"
    chmod 600 "$HOME/.ssh/config"
    echo "  SSH config → ~/.ssh/config"
    echo "  ℹ️  Add machine-specific hosts to ~/.ssh/config.local"
  fi
...
main() {
  ...
  # Global git hooks + platform config
  setup_git_hooks
  setup_git_platform_config
  setup_git_ignore
